...
│                                                                  │
│  ┌─────────────────────┐                                         │
│  │   Python Agent      │  psutil + psycopg2 + subprocess         │
...
| `PostgreSQLMetricsCollector` | `psycopg2` + `pg_stat_*` | connections, size, transactions, deadlocks |
| `ClaudeCodeMetricsCollector` | `~/.claude/projects/` + логи | tokens, sessions |
...
Схема создаётся автоматически OTel Collector (`create_schema: true`). Таблицы:
...
|---|---|---|---|---|
| `postgres.connections` | Gauge | `{connections}` | `state` | `pg_stat_activity` |
| `postgres.database.size` | Gauge | `By` | `database` | `pg_database_size()` |
| `postgres.tables.count` | Gauge | `{tables}` | — | `information_schema.tables` |
| `postgres.transactions` | Counter | `{transactions}` | `database`, `type` (commit, rollback) | `pg_stat_database` |
| `postgres.cache.hit_ratio` | Gauge | `1` | — | `pg_stat_database` blks_hit |
| `postgres.deadlocks` | Counter | `{deadlocks}` | — | `pg_stat_database` |
| `postgres.slow_queries` | Gauge | `{queries}` | — | `pg_stat_activity` > 1s |

PostgreSQL включается автоматически при наличии `psycopg2`. Если коннект упал — переподключается при следующем сборе.

### Claude Code метрики

| Метрика | Тип | Unit | Атрибуты | Источник данных |
|---|---|---|---|---|
| `claude.tokens.today` | Gauge | `{tokens}` | `date` | `~/.claude/projects/` + логи |
| `claude.tokens.weekly` | Gauge | `{tokens}` | `period=7d` | агрегат за 7 дней |
| `claude.sessions.today` | Gauge | `{sessions}` | `date` | файлы сессий по mtime |
| `claude.is_running` | Gauge | `1` | — | `psutil.process_iter` |

Агент сканирует данные не чаще 1 раза в минуту. Цепочка источников: `~/.claude/usage.json` → `~/.claude/projects/*.json` → лог-файлы.
...
| `slow_queries` | slow queries > 0 | `postgres.slow_queries` |
| `deadlocks` | deadlocks > 0 | `postgres.deadlocks` |
| `low_cache_hit` | cache hit ratio < 90% | `postgres.cache.hit_ratio` |
...
| `PostgresSlowQueries` | warning | `postgres_slow_queries > 0` | 2m |
| `PostgresDeadlocks` | critical | `increase(postgres_deadlocks[5m]) > 0` | — |
| `PostgresLowCacheHit` | warning | `postgres_cache_hit_ratio < 0.9` | 10m |
| `PostgresHighConnections` | warning | `sum(postgres_connections) > 80` | 5m |
...
По умолчанию агент читает из `~/.claude/`. Переопределяется через:
```env
CLAUDE_CONFIG_PATH=/path/to/config.json
CLAUDE_USAGE_PATH=/path/to/usage.json
```
...
- Python 3.8+
- (опционально) PostgreSQL, доступный с хоста
...
| OTel zPages | http://localhost:55680/debug/tracez | — |
...
| 55680 | OTel Collector zPages | HTTP |
...
# Rate транзакций PostgreSQL (commits per second)
rate(postgres_transactions{type="commit"}[5m])

# Cache hit ratio ниже порога
postgres_cache_hit_ratio < 0.9

# Метрики с DQ-флагом аномалии (если dq.alert пробрасывается в метку)
{dq_alert!=""}

# Токены Claude за сегодня
claude_tokens_today
...
### PostgreSQL метрики не собираются
...
# psycopg2 установлен?
source venv/bin/activate && python3 -c "import psycopg2; print(psycopg2.__version__)"

# Смотри вывод агента при старте — будет WARNING если psycopg2 не найден
```
...
│
├── agent/
│   ├── workstation_agent.py         # Python Agent (OTel SDK + psutil + psycopg2)
│   └── requirements.txt             # opentelemetry-sdk, psutil, psycopg2-binary
│
├── collector/
...
│   └── alerts/
│       └── alerts.yaml              # Правила алертов (system, postgres, claude, docker)
│
├── clickhouse/
│   └── config/
...
└── grafana/
    ...
    └── dashboards/
        └── workstation.json         # Дашборд мониторинга рабочей станции
```
